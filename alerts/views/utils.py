...
from alerts.models import ReportOld, Station, Report, Sensor
...
def create_report_from_params(params: dict) -> ReportOld:
	"""
	...
	return ReportOld.objects.create(
...
# def extract_csv():
# 	df = pd.read_csv('C:/Users/GAMER/Documents/GitHub/Herbario-Virtual/alerts/views/test_dados.csv', )
# 	df['datahora'] = pd.to_datetime(df['datahora'])
# 	variaveis = ['tmed', 'umidade']
# 	for variavel in range(len(variaveis)):
# 		df[f'{variaveis[variavel]}'] = pd.to_numeric(df[f'{variaveis[variavel]}'], errors='coerce')
# 	s = Sensor.objects.get(id=5)
# 	object_list = []
# 	# for n in range(len(df)):
# 	# 	valor = float(df.loc[n]['tmed'])
# 	# 	datahora = df.loc[n]['datahora'].to_pydatetime()
# 	# 	data = make_aware(datahora)
# 	# 	r = Report(value=valor, sensor=s, time=data)
# 	# 	object_list.append(r)
# 	# s.report_set.set(object_list, bulk=False)
