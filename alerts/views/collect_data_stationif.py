...
from alerts.models import Station, Sensor, Reading
...
def collect_data_stationif(request):
    if request.method == "POST":
        ...
        report_dht_h = Reading(value=dht_h, time=now, sensor=dht_h_station)
        report_dht_t = Reading(value=dht_t, time=now, sensor=dht_t_station)
        report_ldr = Reading(value=ldr, time=now, sensor=ldr_station)
        report_rain = Reading(value=rain, time=now, sensor=rain_station)
        report_soil = Reading(value=soil, time=now, sensor=soil_station)
