...
def get_mathmodels(request, date_filter):
    ...
    for mathmodel in mathmodels:
        ...
        if mathmodel.mathmodelresult_set.all():
            ...
            if date == "day":
                results = mathmodel.mathmodelresult_set.all().filter(date__day=last_mathmodel_result_date.day, date__year=last_mathmodel_result_date.year, date__month=last_mathmodel_result_date.month).values('date', 'value')
                ...
                results = mathmodel.mathmodelresult_set.all().filter(date__range=[start_week, end_week]).values('date', 'value').annotate(
                ...
                results = mathmodel.mathmodelresult_set.all().filter(date__year=last_mathmodel_result_date.year).filter(date__month=last_mathmodel_result_date.month).annotate(
                    value_float=Cast('value', output_field=FloatField())) \
                    .annotate(day=TruncDay('date')) \
                    .values('day') \
                    .annotate(avg_value=Avg('value_float')) \
                ...
                results = mathmodel.mathmodelresult_set.all().filter(date__year=last_mathmodel_result_date.year).annotate(
                    value_float=Cast('value', output_field=FloatField())) \
